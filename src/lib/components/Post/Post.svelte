...
<Saos
  animation={`fade-in-top 400ms cubic-bezier(0.390, 0.575, 0.565, 1.000) ${
    index / 10
  }s both`}
>
  <a class="w-full flex flex-col group" href={url}
    ><h2 class="text-gray-900 font-header font-bold text-2xl mb-2">
      {title}
    </h2>
    <p class="text-gray-500">
      {desc}
    </p>
    <div class="text-accent mt-1 flex items-center font-header">
      Read<svg
        class="h-3 ml-2 transition-transform transform-gpu duration-150 group-hover:translate-x-1"
        height="11"
        viewBox="0 0 7 11"
        fill="none"
